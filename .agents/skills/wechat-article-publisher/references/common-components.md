...
用户要求正文和标题不变时，用本组件承载 Markdown 的 `#` 原题。只替换颜色为主题标题色与主色，不改写、不拆词、不新增副标题；一篇只出现一次。

```html
<section style="margin:0 0 32px;padding:24px 20px 20px;border-bottom:3px solid #DC2626;background:#FFFFFF;">
  <p style="margin:0;font-size:24px;font-weight:900;color:#1C1917;line-height:1.35;letter-spacing:-0.5px;">
    <span leaf="">{{原文一级标题}}</span>
  </p>
</section>
```

主题有能够原样承载完整标题的封面或标题组件时优先使用主题组件；主题要求重写标题、增加标签或副标题时，改用本组件。
