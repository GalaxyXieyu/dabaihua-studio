...
class LeafChecker(HTMLParser):
    ...
    def __init__(self):
        ...
        self.half_punct = []      # (文本片段, 是否生成区) —— 疑似半角标点
    ...
    def handle_data(self, data):
        ...
        if self.code_depth == 0 and (HALF_PUNCT.search(text)
            ...
            snippet = text[:24] + ("…" if len(text) > 24 else "")
            self.half_punct.append((snippet, self.generated_depth > 0))


def validate(html, name="<input>", allow_source_punctuation=False):
    errors, warnings = [], []
    ...
    punctuation_hits = (
        [item for item in checker.half_punct if item[1]]
        if allow_source_punctuation
        else checker.half_punct
    )
    if punctuation_hits:
        sample = "；".join(f"「{snippet}」" for snippet, _ in punctuation_hits[:5])
        warnings.append(
            f"{len(punctuation_hits)} 处正文疑似半角标点/英文引号，应改中文全角"
            f"（代码块内不计）。例：{sample}")
...
def main():
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="正文冻结时保留原稿半角标点，不把它报告为 warning",
    )
    args = ap.parse_args()
    ...
    errors, warnings, leaf_n = validate(
        html, name, allow_source_punctuation=args.allow_source_punctuation
    )
