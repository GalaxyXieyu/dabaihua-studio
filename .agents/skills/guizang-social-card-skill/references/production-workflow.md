...
**内容生产（本 vault / SuperOne）优先**：输出到文章目录，不要默认 `local-tests/` 或 vault 根 `visuals/`：

```text
content/projects/<content-slug>/
├── covers/    # WeChat 16:9 / 1:1, Bilibili, etc.
├── xhs/       # Xiaohongshu 3:4 series (rendered PNGs/JPGs)
└── imgs/      # optional in-article images
```
...
If the user explicitly names another output folder, use that folder instead. Do not create generated task folders or rendered assets in the skill root next to `SKILL.md`. Root-level folders such as `social-card-*`, `livephoto-*`, `wechat-*`, `output/`, vault-root `visuals/xhs|covers` for new content work, and loose `.png` / `.jpg` / `.mov` / `.pvt` artifacts are not allowed for new work.
