...
**SuperOne / 本 vault 内容生产（默认优先）**：若任务属于某篇文章，成品写到文章目录，不要写 skill 的 `local-tests/`，也不要写仓库根 `visuals/`：

```
content/projects/<content-slug>/
├── covers/     # 公众号 16:9 / 1:1、B站封面
├── xhs/        # 小红书 3:4 系列渲染结果
└── imgs/       # 其他文内图（若需要）
```

**Skill 自测 / demo**：才用 `.agents/skills/guizang-social-card-skill/local-tests/<slug>/`（或用户显式指定的输出目录）。

Do not create root-level task folders such as `social-card-*`, `livephoto-*`, `wechat-*`, `output/`, or loose rendered assets next to `SKILL.md`.
...
- **与 visual skill 对齐的安全区**：若用 AI 位图做公众号头条（16:9 再裁列表），主副标题必须 **画面正中**，禁止顶栏标题；列表裁切约 2.35:1 中间横条（2026-07-21 实测）。HTML `21:9` 本身更接近列表比例，标题仍宜水平+垂直居中，四角不放关键字。详见 `.agents/skills/visual/SKILL.md` v1.2。
...
- **内容生产**：输出到 `content/projects/<content-slug>/{covers,xhs,imgs}/`（先确认文章文件夹存在）。
- **Skill 自测 / demo**：才建 `local-tests/<slug>/`，或用户显式指定的输出目录。
- Never put generated task folders, rendered images, MOV files, `.pvt` packages, or downloaded sources in the skill root next to `SKILL.md`，也**不要**默认写到 vault 根 `visuals/`。
...
- Never create generated work in the skill root. Content artifacts go under the article folder; skill demos go under `local-tests/<slug>/` or an explicit user path. Root-level generated folders like `social-card-*`, `livephoto-*`, `wechat-*`, vault-root `visuals/xhs|covers` for new work, and loose output assets are forbidden.
