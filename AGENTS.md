...
- 生产路由：新选题先用 `positioning`；主稿用 `writer`；结构级去 AI 味用 `dabaihua-humanizer`；发布前用 `qa`；多平台文本衍生用 `derive`，单独的小红书正文用 `xhs`；普通配图与封面用 `visual`，成套社交卡和公众号封面对用 `guizang-social-card-skill`。
